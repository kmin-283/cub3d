...
/*   Updated: 2020/05/27 10:08:49 by kmin             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned int	fill_sp_tex(t_player *p, double x, double scale, int i)
{
	...
	if (p->map.map[(int)p->sprite[i].y][(int)p->sprite[i].x] == '2')
		color = p->tex.sp[(int)p->spr.x_cor + p->sprite[i].start_p
			+ (int)(p->spr.y_cor) * 64];
	else if (p->map.map[(int)p->sprite[i].y][(int)p->sprite[i].x] == '3')
		color = p->tex.sp_d[(int)p->spr.x_cor + p->sprite[i].start_p
			+ (int)(p->spr.y_cor) * 64];
	else if (p->map.map[(int)p->sprite[i].y][(int)p->sprite[i].x] == '4')
		color = p->tex.sp_t[(int)p->spr.x_cor + p->sprite[i].start_p
			+ (int)(p->spr.y_cor) * 64];
	p->spr.y_cor += scale;
	if (p->spr.y_cor > 64)
		p->spr.y_cor = 0;
